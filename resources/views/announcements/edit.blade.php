<x-app-layout>
    ...
    <div class="py-12 lg:px-10">
        <form class="max-w-sm mx-auto" action="{{ route('announcements.update', $announcement) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-5">
                ...
                <select name="skill_ids[]" multiple="multiple" class="js-example-basic-multiple bg-blue-50 border border-blue-500 text-blue-900 placeholder-blue-700 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-blue-100 dark:border-blue-400">
                    ...
                    @foreach ($skills as $skill)
                        <option value="{{ $skill->id }}" {{ in_array($skill->id, $announcement->skills->pluck('id')->toArray()) ? 'selected' : '' }}>{{ $skill->skill }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <input type="file" name="image" class="bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-red-100 dark:border-red-400" value="{{ old('image', $announcement->image) }}">
            </div>
